...
FMortalCryPackager::FMortalCryPackager()
{
}
...
void FMortalCryPackager::OpenPluginPackager(TSharedRef<IPlugin> Plugin)
{
...
bool FMortalCryPackager::IsAllContentSaved(TSharedRef<IPlugin> Plugin)
{
...
void FMortalCryPackager::PackagePlugin(TSharedRef<class IPlugin> Plugin, const FString& OutputDirectory)
{
#if PLATFORM_WINDOWS
	...
	IUATHelperModule::Get().CreateUatTask(CommandLine, PlatformName, PackagingText,
		PackagingText, FMortalCryEditorStyle::Get().GetBrush(TEXT("SimpleUGCEditor.PackageUGCAction")),
		[ReleaseVersion, PlatformName, FriendlyName](FString TaskResult, double TimeSec) {});
}

void FMortalCryPackager::FindAvailableGameMods(TArray<TSharedRef<IPlugin>>& OutAvailableGameMods)
{
...
void FMortalCryPackager::GeneratePackagerMenuContent_Internal(class FMenuBuilder& MenuBuilder, const TArray<TSharedPtr<FUICommandInfo>>& Commands)
{
	for (TSharedPtr<FUICommandInfo> Command : Commands)
	{
		MenuBuilder.AddMenuEntry(Command, NAME_None, TAttribute<FText>(), TAttribute<FText>(), FSlateIcon(FMortalCryEditorStyle::GetStyleSetName(), "SimpleUGCEditor.Folder"));
	}
}

void FMortalCryPackager::GeneratePackagerMenuContent(class FMenuBuilder& MenuBuilder)
{
...
TSharedRef<SWidget> FMortalCryPackager::GeneratePackagerComboButtonContent()
{
	...
	for (int32 Index = 0; Index < UGCCommands.Num(); ++Index)
	{
		GameModActionsList->MapAction(
			UGCCommands[Index],
			FExecuteAction::CreateRaw(this, &FMortalCryPackager::OpenPluginPackager, AvailableGameMods[Index]),
			FCanExecuteAction()
		);
	}
...
void FMortalCryPackager::GetAvailableUGCCommands(const TArray<TSharedRef<IPlugin>>& AvailableUGC)
{
	if (UGCCommands.Num() > 0)
	{
		// Unregister UI Commands
		FMortalCryEditorCommands::Get().UnregisterUGCCommands(UGCCommands);
	}
	UGCCommands.Empty(AvailableUGC.Num());

	UGCCommands = FMortalCryEditorCommands::Get().RegisterUGCCommands(AvailableUGC);
}
