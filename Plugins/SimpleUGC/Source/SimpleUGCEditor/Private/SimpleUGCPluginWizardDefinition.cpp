...
void FSimpleUGCPluginWizardDefinition::PopulateTemplatesSource()
{
	...
	FText BaseCodeDescription = LOCTEXT("BaseCode_Description", "Creates an empty C++ mod. This is overwritten if another code template is also selected.");
	...
	FText WeaponCodeDescription = LOCTEXT("WeaponCode_Description", "Creates a new Weapon class in C++.");
	
	FText NewGunName = LOCTEXT("BaseWeapon_Label", "New Gun");
	FText NewGunDescription = LOCTEXT("BaseWeapon_Description", "Creates a new gun, which can be used as a base to create more guns.");
	...
	TemplateToIconMap.Add(NewGunName.ToString(), TEXT("Pistol128.png"));
}
